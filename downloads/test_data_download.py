...
#    move(os.path.join(testImages.split('.')[0],testImages.split('.')[0]), os.path.join(testImages.split('.')[0], folderDetection_Images))
    
    """============== Prepare data for test semantic ================="""
    folderSemantic_Images = 'test_images_semantic'
    os.makedirs(os.path.join(testImages.split('.')[0], folderSemantic_Images), exist_ok=True)
    fileName = "semanticTestImageList_EAD2019.txt"
    download(url, fileName, blocksize=1)
    
    createSemanticTestImages(fileName, os.path.join(testImages.split('.')[0],folderDetection_Images), 
    ...
    if os.path.isdir(fileName.split('.')[0]):
        ...
        move(os.path.join(fileName.split('.')[0],'test_generalization_EAD2019'), os.path.join(testImages.split('.')[0], 'test_generalization'))
    ...
    import glob
    l_detection= len(glob.glob(os.path.join(testImages.split('.')[0],'test_images_detection', '*.jpg'), recursive = True))
    l_semantic= len(glob.glob(os.path.join(testImages.split('.')[0],'test_images_semantic', '*.jpg'), recursive = True))
    l_generalization= len(glob.glob(os.path.join(testImages.split('.')[0],'test_generalization', '*.jpg'), recursive = True))
